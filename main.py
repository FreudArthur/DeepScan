...
col_a.metric("Visages enrôlés", "128", "+4 aujourd'hui")
col_b.metric("Précision estimée", "97.3%", "+0.8%")
col_c.metric("Seuil actif", f"{score_min:.2f}", "personnalisé")
...
if menu == "Accueil":
    ...
    with left:
        ...
        st.markdown(
            ...
            "2. Lance une vérification avec caméra ou image.\n"
            "3. Affiche les correspondances et le score de confiance."
        )
        st.button("Lancer la caméra", use_container_width=True)

    with right:
        st.markdown('<div class="soft-card">', unsafe_allow_html=True)
        st.markdown("#### Aperçu visuel")
        st.image("data/images/bill.jpg", use_container_width=True)
    ...
    with c1:
        ...
        if st.button("Enregistrer le profil"):
            if nom and identifiant and image_ajout:
                st.success("Profil prêt à être indexé.")
            else:
    ...
    with c2:
        ...
        if image_ajout:
            ...
            st.info("Ajoute une image pour voir l'aperçu.")
        st.markdown('</div>', unsafe_allow_html=True)


elif menu == "Vérification":
    st.markdown("### Vérifier une identité")
    ...
    with check_col_1:
        ...
        st.caption(f"Mode temps réel: {'activé' if mode_temps_reel else 'désactivé'}")
        if st.button("Lancer la vérification"):
            if image_test:
                st.success("Analyse lancée. Branche ici ta logique de prédiction.")
            else:
